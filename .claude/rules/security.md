...
If secrets are needed: ask for mock values, stub data, or environment variable names only.
...
- If `.env` exists but `.env.example` doesn't -- flag to user and create one
...
3. If audit finds issues -- report to user before proceeding, suggest `npm audit fix` or manual resolution

Lockfile discipline:
- NEVER run install without a lockfile (`package-lock.json` / `pnpm-lock.yaml` / `yarn.lock`)
- If lockfile is missing -- flag to user, do not blindly generate one
...
- Single-maintainer packages with sudden version bumps = suspicious, flag to user
